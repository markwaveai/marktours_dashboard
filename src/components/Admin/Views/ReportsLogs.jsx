    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold text-gray-800">Reports & Audit Logs</h2>
                <button className="bg-gray-800 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-900">Generate New Report</button>
            </div>
                                ...
                                <tr key={report.id} className="hover:bg-gray-50 transition">
                                    <td className="px-6 py-4 font-medium text-gray-900 flex items-center gap-3">
                                        <span className="text-xl">
                                            {report.type === 'PDF' ? '📄' : report.type === 'CSV' || report.type === 'XLSX' ? '📊' : '📝'}
                                        </span>
                                        {report.name}
                                    </td>
                                    <td className="px-6 py-4">{report.date}</td>
                                    <td className="px-6 py-4">
                                        <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded text-xs font-bold">{report.type}</span>
                                    </td>
