        ...
        <table className="w-full text-sm whitespace-nowrap">
          ...
          <tbody>
            {filteredUsers.map((u, i) => (
              <tr
                key={u.id}
                className="border-t border-gray-50 hover:bg-indigo-50/30 transition-all duration-200 group"
              >
                ...
                <td
                  className="px-4 py-3 cursor-pointer select-none hover:bg-indigo-50 transition-colors"
                ...
                <td className="px-4 py-3 relative group">
                  ...
                  <div className={`hidden group-hover:block absolute z-[60] ${i < 3 ? 'top-full mt-2' : 'bottom-full mb-2'} left-0 w-64 bg-white border border-indigo-100 rounded-xl shadow-2xl p-4 animate-in fade-in slide-in-from-bottom-2 duration-200`}>
                    ...
                    <div className="space-y-2 mb-4">
                      {[1, 2, 3, 4, 5].map((monthOffset) => {
                        try {
                          const baseDate = parseISO(u.nextEmiDate);
                          const futureDate = addMonths(baseDate, monthOffset - 1);
                          return (
                            <div key={monthOffset} className="flex justify-between text-xs items-center">
                              <span className="text-gray-500">{format(futureDate, "MMM yyyy")}</span>
                              <span className="font-semibold text-gray-800">{u.emiAmount}</span>
                            </div>
                          );
                        } catch (e) {
                          return null;
                    ...
                    <div className="pt-3 border-t border-gray-100 mt-2">
                      ...
                      <div className="flex justify-between text-xs font-bold">
                        ...
                        <span className="text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded">12th Month Wave</span>
                      </div>
                    </div>
                  </div>
                </td>
